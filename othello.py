...
root = Tk()
screen = Canvas(root, width=700, height=800, background="#222",highlightthickness=0)
screen.pack()
...
class Board:
	...
	def update(self):
		...
		if not self.won:
			...
			screen.create_text(250,550,anchor="c",font=("Consolas",20), text="The game is not done! play again then it will done")
...
#Separating the use of heuristics for early/mid/late game.
...
def valid(array,player,x,y):
	...
	if array[x][y]!=None:
		...
		#Generating the list of neighbor
		neighbour = False
		neighbours = []
		for i in range(max(0,x-1),min(x+2,8)):
			for j in range(max(0,y-1),min(y+2,8)):
				if array[i][j]!=None:
					neighbour=True
					neighbours.append([i,j])
		#If there's no neighbors, it's an invalid move
		if not neighbour:
			return False
		else:
			#Iterating through neighbor to determine if at least one line is formed
			valid = False
			for neighbour in neighbours:
				...
				#If the neighbor colour is equal to your colour, it doesn't form a line
				#Go onto the next neighbor
				if array[neighX][neighY]==colour:
					continue
				else:
